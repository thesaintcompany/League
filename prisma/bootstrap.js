...
const demoNames = require("./demoNames.js");
...
async function ensureReferees() {
  const REFEREES_30 = [
    ...
    },
    {
      ...
      bio: "Arbitru central cu peste 100 de meciuri  e.",
      image: "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=400&auto=format&fit=crop&q=80",
      coverPhotoUrl: "https://images.unsplash.com/photo-1508098682722-e99c43a406b2?w=800&auto=format&fit=crop&q=80",
    },
    {
    ...
    },
    {
      ...
      bio: "Observator   și formator al noilor generații de arbitri.",
      image: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=400&auto=format&fit=crop&q=80",
      coverPhotoUrl: "https://images.unsplash.com/photo-1508098682722-e99c43a406b2?w=800&auto=format&fit=crop&q=80",
    },
    {
      ...
      bio: "Fost arbitru   de legendă, consultant pe regulamente  e de joc.",
      image: "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=400&auto=format&fit=crop&q=80",
      coverPhotoUrl: "https://images.unsplash.com/photo-1574629810360-7efbbe195018?w=800&auto=format&fit=crop&q=80",
    },
  ];
...
async function ensureVenues(arenaOwnerId) {
  const TIMIS_VENUES = [
    ...
    },
    {
      ...
      specs: "Sală  ă cu parchet omologat, 1.400 de locuri, baschet, volei, futsal, handbal",
      sport: "multifunctional",
      surface: "Parchet",
      capacity: 1400,
      floodlights: true,
  ...
  for (const item of ALL_VENUES) {
    const { matchNames, ...venueData } = item;
    const existingVenue = await prisma.venue.findFirst({
      where: {
        OR: [
          { name: venueData.name },
          ...(matchNames ? matchNames.map((n) => ({ name: n })) : []),
        ],
      },
    });
    ...
    if (!existingVenue) {
      ...
      await prisma.venue.update({
        ...
        data: {
          ...
          status: venueData.status || "activ",
          address: existingVenue.address || venueData.address,
          specs: existingVenue.specs || venueData.specs,
          sport: existingVenue.sport || venueData.sport,
          surface: existingVenue.surface || venueData.surface,
          capacity: existingVenue.capacity || venueData.capacity,
          floodlights: existingVenue.floodlights ?? venueData.floodlights,
          pricePerHour: existingVenue.pricePerHour ?? venueData.pricePerHour,
          imageUrl: existingVenue.imageUrl || venueData.imageUrl,
          galleryImages: existingVenue.galleryImages || venueData.galleryImages,
          phone: existingVenue.phone || venueData.phone,
          website: existingVenue.website || venueData.website,
          googleMapsUrl: existingVenue.googleMapsUrl || venueData.googleMapsUrl,
          rating: existingVenue.rating ?? venueData.rating,
          reviewCount: existingVenue.reviewCount ?? venueData.reviewCount,
          isActive: true,
        },
      });
      updatedCount++;
  ...
  console.log(`[seed] ensured ${ALL_VENUES.length} national stadiums, polyvalent halls, and county arenas (${createdCount} created, ${updatedCount} updated)`);
}
...
async function ensureDemoChampionship(ownerId) {
  ...
  // 8 Elite Teams (randomized Romanian club names)
  const teamsData = Array.from({ length: 8 }, () => demoNames.randomTeam());
  ...
  // Randomized Romanian player names distributed across the demo teams.
  for (let i = 0; i < createdTeams.length; i++) {
    const team = createdTeams[i];
    const numPlayers = demoNames.randomInt(14, 22);
    for (let j = 0; j < numPlayers; j++) {
      const p = demoNames.randomPlayer(team.shortName);
