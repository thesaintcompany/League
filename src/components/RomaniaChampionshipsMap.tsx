    ...
    <div className="space-y-10 font-body">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-6 space-y-6">
          <div className="card p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-lg space-y-4">
            <div className="flex justify-between items-center pb-3 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-2">
                <span className="text-xl">🗺️</span>
                <h3 className="font-headline font-bold text-base text-slate-900 dark:text-white">
                  Desen Teritorial România
                </h3>
              </div>
              <span className="text-[11px] font-label font-bold text-lime-600 dark:text-lime-400 uppercase bg-lime-400/10 px-3 py-1 rounded-full border border-lime-400/30">
                Apasă pe hartă pentru selecție
              </span>
            </div>
            ...
            <div className="pt-3 border-t border-slate-100 dark:border-slate-800 space-y-2">
              <span className="text-[10px] font-label font-bold uppercase tracking-widest text-slate-400 block">
                Săritură Rapidă pe Regiuni &amp; Județe Mari:
              </span>
                ...
                {["Timiș", "Cluj", "București", "Iași", "Brașov", "Dolj", "Constanța", "Bihor", "Argeș", "Prahova"].map((cName) => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1 rounded-xl text-xs font-bold font-label transition fluid-press ${
                        isSel
        ...
        <div className="lg:col-span-6 space-y-6">
          ...
          <div className="card p-6 bg-white dark:bg-slate-900 border-2 border-lime-400/40 rounded-3xl shadow-xl space-y-5 relative overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
              <div className="space-y-1">
                ...
                <h2 className="text-3xl font-black italic font-headline uppercase text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
              ...
              <Link
                href="/dashboard/new"
                className="px-4 py-2.5 rounded-2xl bg-lime-400 hover:bg-lime-300 text-slate-950 font-headline font-black text-xs uppercase tracking-wider shadow-md flex items-center gap-1.5 transition fluid-press shrink-0"
              >
            ...
            {/* County Selector Tabs: Championships vs Venues */}
            <div className="flex items-center justify-between gap-3">
              <div className="flex gap-2 bg-slate-100 dark:bg-slate-800 p-1.5 rounded-2xl border border-slate-200 dark:border-slate-700">
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-bold font-label transition fluid-press flex items-center gap-1.5 ${
                ...
                >
                  <span>🏟️</span> Arene &amp; Stadioane ({countyVenues.length})
                </button>
              </div>

              {/* Quick Search within County */}
            ...
            {activeTab === "championships" && (
              <div className="space-y-4 max-h-[580px] overflow-y-auto pr-1">
                {filteredChampionships.length === 0 ? (
                  <div className="p-10 rounded-3xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 text-center space-y-3">
                  ...
                  filteredChampionships.map((champ) => {
                    ...
                    return (
                      <div
                        key={champ.id}
                        className="card p-5 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/80 rounded-2xl shadow-sm hover:border-lime-400/80 transition space-y-3 group"
                      >
                        <div className="flex items-center justify-between gap-2 pb-2.5 border-b border-slate-200/80 dark:border-slate-700">
                          <div className="flex items-center gap-2">
                            <span
                            ...
                            <span className="px-2 py-0.5 rounded-full bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-slate-200 text-[10px] font-bold font-label uppercase">
                              {champ.sport}
                            </span>
                          </div>
                        ...
                        <div>
                          <h4 className="font-headline font-bold text-lg text-slate-900 dark:text-white group-hover:text-lime-600 dark:group-hover:text-lime-400 transition-colors">
                            {champ.name}
                          </h4>
                          {champ.description && (
                        ...
                        <div className="pt-2 flex flex-wrap items-center justify-between gap-3 text-xs font-label">
                          ...
                          <div className="flex items-center gap-2">
                            <Link
                              href="/brackets"
                              className="px-3 py-1.5 rounded-xl bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-900 dark:text-white font-bold text-[11px] uppercase font-label transition flex items-center gap-1"
                            >
                              🌳 Harta Meciuri
                            </Link>
                            <Link
                              href={`/campionat?id=${champ.id}`}
                              className="px-3.5 py-1.5 rounded-xl bg-lime-400 hover:bg-lime-300 text-slate-950 font-black text-[11px] uppercase font-headline shadow-sm transition flex items-center gap-1"
                            >
                              Clasament ↗
                            </Link>
                          </div>
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="text-xs font-black font-mono text-lime-600 dark:text-lime-400">
                          Capacitate: {v.capacity.toLocaleString()} Locuri
                        </span>
                      </div>
                      ...
                      <div className="pt-2 border-t border-slate-200/80 dark:border-slate-700 flex justify-between items-center">
                        ...
                        <Link
                          href={`/venues/${v.id}`}
                          className="px-3.5 py-1.5 rounded-xl bg-slate-900 dark:bg-lime-400 text-white dark:text-slate-950 font-headline font-black text-[11px] uppercase tracking-wider hover:bg-lime-500 transition flex items-center gap-1 shadow-sm"
                        >
                          Detalii Arenă ↗
                        </Link>
                      </div>
